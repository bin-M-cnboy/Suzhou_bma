...
def textualize_graph(i,line):
    ...
    with open(f'{path}/KG_QA1_vBGE/des/{i+2700}.json', 'w') as file:
...
def step_one():
    ...
    os.makedirs(f'{path}/KG_QA1_vBGE/des', exist_ok=True)

    with open(f'{path}/triples3000.txt', 'r') as file:
        for i,line in enumerate(file):
            if i < 10:
                continue
            nodes, edges = textualize_graph(i-10,line)
...
def step_two():
    ...
    os.makedirs(f'{path}/KG_QA1_vBGE', exist_ok=True)
    os.makedirs(f'{path}/KG_QA1_vBGE/graph', exist_ok=True)
    ...
    for i in range(len(nodeslist)):
        ...
        torch.save(data, f'{path}/KG_QA1_vBGE/graph/{i+2700}.pt')

    # encode questions
    os.makedirs(f'{path}/questions_emb_vBGE', exist_ok=True)

    data_path = '../CRUD/QA1.json'
    ...
    for i in range(len(data)):
        ...
        torch.save(question_emb, f'{path}/questions_emb_vBGE/{i}.pt')
...
def encode_doc():
    ...
    os.makedirs(f'{path}/KG_QA1_vBGE', exist_ok=True)
    os.makedirs(f'{path}/KG_QA1_vBGE/doc_text', exist_ok=True)
    os.makedirs(f'{path}/KG_QA1_vBGE/doc_emb', exist_ok=True)
    ...
    torch.save(doc_emb_list, f'{path}/KG_QA1_vBGE/doc_emb/{3000}.pt')
    with open(f'{path}/KG_QA1_vBGE/doc_text/{3000}.pkl', 'wb') as file:
