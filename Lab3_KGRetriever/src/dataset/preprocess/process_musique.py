...
def step_one():
    ...
    tmp_deslist = []
    for i in range(100):
        with open (f'{path}/tempfile/{i}.json') as file:
...
def encode_question():
    ...
    index = -1
    data_path = '../musique/musique_ans_v1.0_dev.jsonl'
    with open(data_path, 'r') as file:
        for line in file:
            ...
            torch.save(question_emb, f'{path}/questions_emb_vRobert/{index}.pt')
...
def encode_doc():
    ...
    with open(data_path, 'r') as file:
        for line in file:
            ...
            if index == 100:
                break
            paragraphs = json.loads(line)['paragraphs']
